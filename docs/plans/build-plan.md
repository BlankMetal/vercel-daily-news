...
5. **Root layout** — Set up `app/layout.tsx` with a custom font (via `next/font`), global Tailwind styles, and root metadata with Open Graph defaults (title template `"%s | Vercel Daily News"`, default description from the publication config endpoint).
...
1. **Hero section** — Static Server Component with headline text, a description, and a visual element (could be the first featured article's image or an illustration). Style it prominently.
